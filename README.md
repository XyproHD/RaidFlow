...
- **Raid bearbeiten / Raid ansetzen:** Gleiche Filter wie in der DatePicker-Ansicht (Twinks, Rollen, Werktag/Wochenende). **Teilnehmerliste in drei Spalten:** „Gesetzt“, „Ersatz“, „Anmeldung“; über jeder Spalte ein **Add**-Button für nicht angemeldete Spieler. **Chars als Buttons** mit Steuerung: ➕ (Gesetzt), 🪑 (Ersatz), ➖ (nur angemeldet). **Off-Spec-Symbol:** Wechsel Main/Off-Spec nur für diesen Raid und nur wenn der Spieler es bei der Anmeldung erlaubt hat. Notiz für Raidleader: 📒 mit Tooltip bei Mouseover. Hinter jedem Char-Namen **Teilnahmestatistik (x/y)** (x = Teilnahmen in diesem Dungeon, y = Teilnahmen aller Dungeons). Grunddaten ändern, Raidleader/Lootmaster aus Pool, Termin verschieben/absagen, Prüfung „2 Gruppen möglich?“, **Raid setzen** (Status „fertig“, Liste veröffentlichen, Discord-Benachrichtigung, Thread-Update).
