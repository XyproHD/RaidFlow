...
- **Dashboard:** Zentrale Übersicht mit:
  ...
  - **Kalender:** Zeitraum als **Kachel- oder Listenansicht** umschaltbar. Zusätzliche Steuerung:
    ...
    - **Kacheln:** Raidname, Uhrzeit, **ein oder mehrere Schlachtzüge** (mit **` / `** getrennt) und Gilde; Zeile **„Anmeldung:“** mit Ampel **🟢** (mehr als 30 Stunden bis zur Anmeldefrist), **🟡** (weniger als 30 Stunden), **🔴** (Frist abgelaufen). Konkretes **Datum und Uhrzeit** der Frist per **Hover** (Tooltip) oder **Klick** (aufklappen). **Raidleader / Gildenmeister:** **⋮**-Menü; beim Öffnen liegt ein **abgedunkeltes Overlay** über der Seite (fixiert, **ohne** dass sich das Layout verschiebt) mit: Anmeldung starten oder bearbeiten (je nachdem), Raid bearbeiten, Raid absagen (nur bei Status **offen**), Raid löschen. Ist der Raid **nicht offen** (z. B. abgesagt): **keine** Anmeldung über die Kacheln; stattdessen Hinweis **„Anmeldung gesperrt“**.
    - **Listenansicht:** Spalten u. a. Termin, **Raid** (Name und **ein oder mehrere Schlachtzüge**, lokalisiert und mit **` / `** getrennt, darunter **`@ Gildenname`**), **Anmeldung bis** (dieselbe Ampel wie in den Kacheln), Status, eigener Status, **Schnellaktion** (➕ **Anmelden** / ➖ **Abmelden**, je nach eigener Anmeldung und nur wenn der Raid **offen** ist), **Aktionen** als **⋮**-Menü wie bei den Kacheln (inkl. Overlay).
    - **Einträge (Kacheln)** zeigen außerdem Status, Notiz-Indikator **📒**, sowie **Anmeldungen** (Zähler/max.) als eigene Zeile.
...
- **Mitglieder** der Gilde einsehen und Raidgruppen zuordnen (in der App oder über Discord-Rollen). An **Charakterkarten** erscheint wie im Profil das **Battle.net-Logo**, wenn der Charakter in der DB mit einer Battle.net-Charakter-ID verknüpft ist.
...
- **Neuer Raid (mehrseitig):** Der Raidplaner wird in mehreren Seiten mit Vor-/Zurück-Navigation geführt. **Seite 1 – Alle Raideinstellungen:** **Schlachtzüge** (mehrere aus der DB **ankreuzbar**); **max. Teilnehmer** wird automatisch aus dem **Maximum** der gewählten Dungeons gesetzt (Feld nur lesbar). Außerdem: Name, Raidleader, Lootmaster, Notiz, Raidtermin, „Anmeldung bis“, Sichtbarkeit der Anmeldeliste (öffentlich oder nur Raidleader), optionale Raidgruppen-Einschränkung, Discord-Channel für Thread, optional Discord-Thread anlegen. In der DB bleibt `dungeon_id` die **primäre** Instanz (erste Auswahl); zusätzliche IDs stehen in **`dungeon_ids`** (JSON-Array). **Seite 2 – Zeitleiste:** Slots 16–03 Uhr (30-Minuten); die **Heatmap** zur Mindestbesetzung erscheint als **farbiger Punkt** pro Slot (schmale Spalte), nicht als vollflächig eingefärbte Zelle. **Filter** (als Schalter/Toggles): **Mains | Beide | Twinks** (genau eine Option, Standard Mains); **Werktag | Wochenende** (beide umschaltbar, mindestens einer aktiv); **Tank | Melee | Ranged | Heiler** (mindestens eine Rolle); **Verfügbarkeit** (Alle | Verfügbar | Eingeschränkt | Nicht verfügbar). Überschrift der Spielerliste: **„Mitglieder“**; Zeilen mit Main/Twink, Klasse, Spec-**Icons** (Offspec in Graustufen), Charname, Badges (Discordname, WD/WE, BNet, Gearscore). **Live-Zähler** für Kapazität: **passende Spieler / max. Teilnehmer** (`x/y`). **Spielerliste** nach Rollen und Klassen gruppiert; **Grün** = im gewählten Zeitraum verfügbar, **Orange** = mind. 1× eventuell, **ausgegraut** = mind. 1 Zeitslot außerhalb.
- **Raid-Ansicht / Dashboard:** Sind mehrere Schlachtzüge am Raid gespeichert, werden die **Namen** (lokalisiert über `rf_dungeon_name`) in einer Zeile mit **` / `** angezeigt (z. B. in der Raid-Detail-Kopfzeile und in Kalender-/Anmeldungslisten).
- **Raid bearbeiten / Raid ansetzen:** Gleiche Filter wie in der DatePicker-Ansicht (Twinks, Rollen, Werktag/Wochenende). **Teilnehmerliste:** Spalten „Gesetzt“, „Ersatz“, „Anmeldung“ sowie **„ID verplant“** (Charaktere, die in einem anderen Raid derselben Instanz bereits „Gesetzt“ sind, können nicht weiter eingeteilt werden). Über den inhaltlichen Spalten **Add**-Button für nicht angemeldete Spieler. **Chars als Buttons** mit Steuerung: ➕ (Gesetzt), 🪑 (Ersatz), ➖ (nur angemeldet). Ist ein Char in **mehreren** Raids (derselben Instanz) angemeldet/gesetzt: **„ID“ rot**; Klick zeigt die Termine. **Spec-Wechsel (⇄):** Main/Off-Spec nur für diesen Raid; **deaktiviert**, wenn der Spieler bei der Anmeldung **„Nur angemeldeter Spec“** (`only_signed_spec` in `rf_raid_signup`) gesetzt hat. **Badges** in der Zeile: **Nur angemeldeter Spec**, **Spieler: kein Reserve** (`forbid_reserve`), zusätzlich zur bisherigen Kennzeichnung „Reserve gesperrt“ durch die Raidleitung. Notiz für Raidleader: 📒 mit Tooltip bei Mouseover. Hinter jedem Char-Namen **Teilnahmestatistik (x/y)**. Grunddaten ändern, Raidleader/Lootmaster aus Pool, Termin verschieben/absagen, Prüfung „2 Gruppen möglich?“, **Raid setzen** (Status „fertig“, Liste veröffentlichen, Discord-Benachrichtigung, Thread-Update).
...
- **Anmelden** (Modus „Anmelden“ / `?mode=signup`): **Teilnahme** (Bin da / Unklar / Reserve) und **Pünktlichkeit** (rechtzeitig / wird knapp / später) als Schalter mit Icons; „wird knapp“ und „später“ setzen `is_late` analog zu einer Verspätungsmeldung. **Bedingungen** des Spielers (optional): **Nur angemeldeter Spec** und **Reserve verbieten** – werden in **`rf_raid_signup`** als `only_signed_spec` und `forbid_reserve` gespeichert und von der API durchgesetzt (z. B. keine Reserve-Anmeldung bei `forbid_reserve`). **Pflichtnotiz** an die Raidleitung bei Unklar, Reserve, „wird knapp“/„später“ (kurzer Text); sonst optional. Oben: **Raid-Meta** (u. a. Anmeldung bis, Plätze, Status, Sichtbarkeit) und **Rollenstatus** je Rolle als `x (y / z)` (Bin da / Unklar / Reserve). **Char + Spec** in einer Zeile (Icons). Im Anmelde-Modus **keine** Teilnehmerliste und **keine** Anmeldehistorie in der UI (Audit-Log im Backend bleibt möglich). Nach Ablauf von „Anmeldung bis“ ist nur noch Reserve möglich. **Instanz-ID:** Schlachtzüge haben ein Reset-Fenster (ID); die ID gilt **pro Character**. Bei weiterer Anmeldung im gleichen Zeitfenster wird ein Hinweis angezeigt. **Parallele Anmeldungen** für denselben Schlachtzug an mehreren Terminen (z. B. Freitag + Samstag) sind möglich – der Raidleader entscheidet die Einteilung.
- **Abmelden** und **Anmeldung bearbeiten** (Typ, Pünktlichkeit, Bedingungen).
- **Liste der angemeldeten Spieler** – in der **Ansicht** „Anzeigen“ (`mode=view`): sichtbar je nach `signup_visibility`; Einträge mit **Verspätung** bzw. **⏱** wo relevant; **Badges** für Spieler-Bedingungen (**Nur angemeldeter Spec**, **Spieler: kein Reserve**); bei gesperrter Reserve durch die Raidleitung weiterhin das bestehende Badge „Reserve gesperrt“ (ohne Doppelung bei `forbid_reserve`). Notizen für Raidleader z. B. mit 📒/Tooltip in der Raidleiter-Ansicht.
...
| [BNET_INTEGRATION.md](BNET_INTEGRATION.md) | Battle.net API: Konfiguration, Gildenverknüpfung, Charakter-Sync, Battle.net-Logo in der UI, Endpunkte |
